...
function HomeImportadas({navigation}) {
  ...
  const carregaProduto = async () => {
    ...
    setProdutos(data.filter(p => p.categoria === 'importadas'));
    setProdutosFiltrados(data.filter(p => p.categoria === 'importadas'));
    setLoading(false);
  };
